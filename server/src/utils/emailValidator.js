 ...
 * @returns {Promise<boolean>} 邮箱是否已存在
 */
export async function checkEmailExists(email) {
  ...
  // 使用 LOWER() 函数进行大小写不敏感比较
  const existing = await db.prepare('SELECT id FROM users WHERE LOWER(email) = LOWER(?)').get(normalized)
  return !!existing
}
